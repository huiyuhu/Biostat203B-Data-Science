...
filter(diamonds, carat == 1) %>%
  summarise(n())
```
* Answer: there are only 23 diamonds are 0.99 carat but 1558 diamonds are 1 carat. I think the reason is that the most measurements for diamonds can not be preicise to 0.01 and seller tend to make the carat integer.
...
ggplot(diamonds, aes(y)) +
  ...
  coord_cartesian(xlim = c(2, 60), ylim = c(0, 100))
```
...
ggplot(diamonds) +
  ...
  facet_wrap(~cut)
```
* Answer: geom_violin() showed smoth curve and distribution but difficult to see where is peak. geom_histogram() can be modified with but hart to make compairision. geom_freqpoly() can showed the trends very well but cannot showed the gaps and exact count. I prefered the geom_freqpoly(), since it's clear to see the distribution and the peak.

6.If you have a small dataset, it’s sometimes useful to use geom_jitter() to see the relationship between a continuous and categorical variable. The ggbeeswarm package provides a number of methods similar to geom_jitter(). List them and briefly describe what each one does.
```{r}
library(ggbeeswarm)
small_diamonds <- filter(diamonds, cut <= 1) %>%
...
```
* The result is super difficult to read because there are so many destinations, which make the plot so mess. The color scale is also difficult to distinguish the pattern of the distribution. In addition, there are also a lot missing values. 
* To improve this result, we may seperate the distination to make the y-axis have less categories. We can filter the most common dest to analyze. In addition, we can filter the missing value from dataset.
```{r}
not_cancelled <- nycflights13::flights %>% 
  drop_na(dep_time)
common_dest <- not_cancelled %>%
  group_by(dest) %>%
    count(sort = T) %>% .[1:10,]

not_cancelled %>% filter(dest %in% common_dest$dest) %>%
    group_by(dest, month) %>%
