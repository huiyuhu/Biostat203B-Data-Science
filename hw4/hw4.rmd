...
* I choose to find the top 10 destination that most frequently delay more than 45 min.
```{r}
LAX_delay30 <- inner_join(LAX2000_dest, airports_tbl,
                          by = c("dest" = "faa"), copy = T) %>%
  ungroup() %>%
  select(dest, n, lat, lon) %>%
  collect()
```
```{r}
LAX_delay30$lat <- as.numeric(LAX_delay30$lat)
LAX_delay30$lon <- as.numeric(LAX_delay30$lon)
usa <- map_data("usa")
gg5 <- ggplot() + 
  geom_polygon(data = usa, aes(x = long, y = lat, group = group),
               fill = "seashell", color = "seashell3") + 
    coord_fixed(1.3) + 
  geom_point(data = LAX_delay30, aes(x = lon, y = lat, size = n, color = dest))
