...
select(Year,`Department Title`, `Job Class Title`, `Total Payments`, `Base Pay`,
  ...
  group_by(Year) %>% arrange( desc( `Total Payments` ), by_group = T )
write_rds(highpay, "highpay.rds")

# Data set for question 3
meanpay <- payroll %>%
  group_by(Year,`Department Title`) %>%
  summarise(`Mean Base Pay` = mean(as.numeric(gsub("\\$", "", `Base Pay`)), 
                                   na.rm = TRUE),
            ...
            `Mean Other Pay` = mean(as.numeric(gsub("\\$", "", 
                                    ...
                                    na.rm = TRUE),
            `Mean Total Pay` =  mean(as.numeric(gsub("\\$", "",`Total Payments`)),
                                    na.rm = TRUE)
            ) %>% group_by(Year) %>%  arrange( desc( `Mean Total Pay` ), Year )
write_rds(meanpay, "meanpay.rds")

medpay <- payroll %>%
  group_by(`Department Title`, Year) %>%
  summarise(`Median Base Pay` = median(as.numeric(gsub("\\$", "", `Base Pay`)), 
                                   na.rm = TRUE),
            ...
            `Median Other Pay` = median(as.numeric(gsub("\\$", "", 
                                    `Other Pay (Payroll Explorer)`)),
                                    na.rm = TRUE),
            `Median Total Pay` =  median(as.numeric(gsub("\\$", "",`Total Payments`)),
                                    na.rm = TRUE)
            ) %>% group_by(Year) %>%  arrange( desc( `Median Total Pay` ), Year )
