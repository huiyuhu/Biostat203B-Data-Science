...
meanpay <- payroll %>%
  ...
  summarise(`Mean Total Pay` =  paste0("$", mean(as.numeric(gsub("\\$", "",`Total Payments`)),
                                    na.rm = TRUE)),
            `Mean Base Pay` = paste0("$", mean(as.numeric(gsub("\\$", "", `Base Pay`)), 
                                   na.rm = TRUE)),
            `Mean Overtime Pay` = paste0("$", mean(as.numeric(gsub("\\$", "",`Overtime Pay`)),
                                       na.rm = TRUE)),
            `Mean Other Pay` = paste0("$", mean(as.numeric(gsub("\\$", "", 
...
medpay <- payroll %>%
  ...
  summarise(`Median Total Pay` =  paste0("$", median(as.numeric(gsub("\\$", "",`Total Payments`)),
                                    na.rm = TRUE)),
            `Median Base Pay` = paste0("$", median(as.numeric(gsub("\\$", "", `Base Pay`)), 
                                   na.rm = TRUE)),
            `Median Overtime Pay` = paste0("$", median(as.numeric(gsub("\\$", "",`Overtime Pay`)),
                                       na.rm = TRUE)),
            `Median Other Pay` = paste0("$", median(as.numeric(gsub("\\$", "", 
                                    `Other Pay (Payroll Explorer)`)),
                                    na.rm = TRUE))
...
cost <- payroll %>% 
  group_by(Year,`Department Title`) %>%
  summarise(`Total Pay` =  paste0("$", sum(as.numeric(gsub("\\$", "",`Total Payments`)),
                                    na.rm = TRUE)),
            `Base Pay` = paste0("$", sum(as.numeric(gsub("\\$", "", `Base Pay`)), 
                                   na.rm = TRUE)),
            `Overtime Pay` = paste0("$", sum(as.numeric(gsub("\\$", "",`Overtime Pay`)),
                                       na.rm = TRUE)),
            `Other Pay` = paste0("$", sum(as.numeric(gsub("\\$", "", 
                                    `Other Pay (Payroll Explorer)`)),
                                     na.rm = TRUE)),
            `Total Cost` = paste0("$", sum(as.numeric(gsub("\\$", "", `Average Benefit Cost`)),
                                       na.rm = TRUE))
...
ui <- fluidPage(
  titlePanel("Employees with Highest Earnings"),
  sidebarLayout(
    sidebarPanel(
...
ui <- fluidPage(
  titlePanel("Payroll Mean and Median"),
  sidebarLayout(
    sidebarPanel(
...
```{r}
library(shinythemes)
totpay <- readRDS("~/biostat-m280-2018-winter/hw3/totpay.rds")
highp <- readRDS("~/biostat-m280-2018-winter/hw3/highpay.rds")
meanp <- readRDS("~/biostat-m280-2018-winter/hw3/meanpay.rds")
medp <- readRDS("~/biostat-m280-2018-winter/hw3/medpay.rds")
cost <- readRDS("~/biostat-m280-2018-winter/hw3/cost.rds")
ui <- navbarPage( theme = shinytheme("cerulean"),
                  "LA City Employee Payroll",
                   ...
                   tabPanel("Who earned most?",
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                numericInput("rows", "Choose number of row:",
                                         10, min = 0, max = 20),
                   ...
                   tabPanel("Which departments earn most?",
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                numericInput("rows1", "Choose number of row:",
                                             5, min = 0, max = 20),
                   ...
                   tabPanel("Which departments cost most?",
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                numericInput("rows2", "Choose number of row:", 5, min = 0, max = 20),
...
newcon = dbConnect(RSQLite::SQLite(), "/home/m280-data/la_parking/LA_Parking_Citations_Extra.sqlite")
str(con)
str(newcon)
dbListTables(con)
dbListTables(newcon)
...
* There are `4044488` tickets; and there are `4044338` tickets (distinct) in this data sets.
