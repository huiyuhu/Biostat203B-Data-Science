...
highpay <- payroll %>% 
select(Year,`Department Title`, `Job Class Title`, `Total Payments`, `Base Pay`, `Overtime Pay`,
       `Other Pay (Payroll Explorer)`) %>% 
  group_by(Year) %>% top_n(100, `Total Payments`)
...
```{r}
ui <- fluidPage(
  numericInput("rows", "How many top highest?", 10),
  selectInput("year", "Which Year?", c("2013" = "2013", "2014" = "2014",
                                       "2015" = "2015", "2016" = "2016",
                                       "2017" = "2017")),
  tableOutput(outputId = "high_pay")
)
server <- function(input, output) {
  output$higy_pay <- renderTable({
    head(q1, input$rows)
  })
}
shinyApp(ui = ui, server = server)
